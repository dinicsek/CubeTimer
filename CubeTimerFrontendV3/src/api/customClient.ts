import { getRefreshEndpointMutationOptions } from './generated/features/auth/auth';
    ...
    try {
        ...
        if (error.status === 401) {
            try {
                ...
                const refreshOptions = getRefreshEndpointMutationOptions();
                if (refreshOptions.mutationFn) {
                    const res = await refreshOptions.mutationFn({ data: {} });
                    ...
                    throw new Error('refreshOptions.mutationFn is undefined');
                }
